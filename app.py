...
# translator helper (rename from "_" to avoid collisions)
...
.neon-header{ display:flex; align-items:center; gap:10px; padding:10px 16px; border-radius:16px;
  ...
  border:2px solid rgba(91,185,255,.35); box-shadow:0 0 16px rgba(61,240,165,.25), inset 0 0 20px rgba(91,185,255,.12);
...
def _extract_text_pdfminer(file_bytes: bytes) -> str:
    ...
    try:
        ...
        try:
            from pdfminer_high_level import extract_text  # some dists alias it
        except Exception:
            return ""
    try:
